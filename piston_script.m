...
%% max forces value computation

% find the max values of the forces
% index 1 - omega_value
% index 2 - t A
% index 3 - max_theta A
% index 4 - max_F A
...
% index 6 - max_theta P
% index 7 - max_F p
max = zeros(3*length(omega_values),7);
...
for i = 1:length(omega_values)
    ...
    for k = 0:2
        ...
        for j = 0:(2*pi()/omega_values(i))/10:2*pi()/omega_values(i)
            ...
            L=L_arr(k+1);
            H=H_arr(k+1);
            if(abs(subs(A)))>max(3*k+i,4)
                max(3*k+i,2)=t;
                max(3*k+i,3)=omega*t;
                max(3*k+i,4)=subs(A);
            end
            if(abs(subs(P)))>max(3*k+i,7)
                max(3*k+i,5)=t;
                max(3*k+i,6)=omega*t;
                max(3*k+i,7)=subs(P);
            end
...
% output the table of maxes
% as of now this will be manually coppied over -- not autogened
disp(table(max(:,1),max(:,2),max(:,3),max(:,4),max(:,5),max(:,6),max(:,7)))
...
if (sum(options.contains("write")))
    ...
    try 
        ...
        append_text(fobj_fn_def.fid, "The following equations are the resultant equations"...
            ...
            + "unwieldy both due to their inherent complexity, hence the more easily understood expressions used throughout the body"...
